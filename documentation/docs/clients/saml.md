...
By default, the entity ID of your application (the Service Provider) will be equal to the [callback URL](../clients.html#3-the-callback-url).
...
By SAML specification, the authentication request must not contain a NameQualifier, if the SP entity is in the format nameid-format:entity. However, some IdPs require that information to be present. You can force a NameQualifier in the request with the `useNameQualifier` parameter:
...
When the assertions need to be processed separately from the response, you can request them to be signed directly using:
...
- Contacting URL endpoints is done using `SAML2HttpClientBuilder` that builds on OpenSAML v4 with minor
...
There are two ways to make the values equal:
...
denoting the priority. The lowest value is the one that will ultimately be used as the configuration. The default implementation has
an effective priority of `100`. Generic providers likely should use something like `50`, while end user implementors should
use `1`.  for example:
