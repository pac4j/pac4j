...
- The `LdapProfileService`, `DbProfileService` and `MongoProfileService` replace the deprecated `LDapAuthenticator`, `DbAuthenticator` and `MongoAuthenticator` to validate username/password and create, update or delete users in an LDAP, in a relational database and in a MongoDB database
...
- Critical security issue since version 1.9.2 on the `NopPasswordEncoder` regarding the `MongoAuthenticator` and the `DbAuthenticator`: upgrading is mandatory
...
- Profile attributes can be checked via the `RequireAnyAttributeAuthorizer`
...
- Upgraded to Java 8 as well as almost all dependency versions
...
- Typed ids are now defined using the full class name (with package): "org.pac4j.oauth.profile.facebook.FacebookProfile#id" instead of "FacebookProfile#id" (use the `getOldTypedId()` method to get the old value)
...
- Added Microsoft Azure AD (OpenID Connect) support
...
- infinite loops when accessing a protected page and authentication fails are now automatically handled by returning a forbidden response (HTTP 403)
...
- Add the ability to handle credential exceptions when retrieving credentials (use <i>BaseOAuthProvider.retrieveCredential</i> method instead of <i>OAuthProvider.getCredential</i> method)
...
- Optimize profile creations for non-CAS usage
