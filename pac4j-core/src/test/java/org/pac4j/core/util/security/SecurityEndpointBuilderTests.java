...
public final class SecurityEndpointBuilderTests implements TestsConstants {
    ...
    public void buildSimpleObjectConfig() {
        ...
        assertAuthorizer(endpoint.getAuthorizers(), authorizer, 1);
        assertMatcher(endpoint.getMatchers(), matcher, 1);
        assertNull(endpoint.getSecurityLogic());
        assertNull(endpoint.getHttpActionAdapter());
    }
    ...
    protected void assertAuthorizersAndMatchers(final Authorizer authorizer1, final Authorizer authorizer2,
        ...
        assertAuthorizer(authorizerNames[0], authorizer1, 2);
        assertAuthorizer(authorizerNames[1], authorizer2, 2);
        final String[] matcherNames = endpoint.getMatchers().split(",");
        assertMatcher(matcherNames[0], matcher1, 2);
        assertMatcher(matcherNames[1], matcher2, 2);
        assertNull(endpoint.getSecurityLogic());
        assertNull(endpoint.getHttpActionAdapter());
    }
    ...
    protected void assertAuthorizer(final String name, final Authorizer authorizer, final int size) {
    ...
    protected void assertMatcher(final String name, final Matcher matcher, final int size) {
