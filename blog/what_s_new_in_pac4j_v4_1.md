...
Version 4.1 will be focused on cleaning (with non-breaking changes) and version 5 as well (with breaking changes).
...
But a simple `setSessionStore` method at the `ProfileManager` will do the same job and remove its need.
...
While this CSRF protection makes sense for stateful web applications, it does not make any sense for stateless web services. So if no `IndirectClient` is configured at the security level,
