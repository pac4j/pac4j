...
However, the library was created to be the foundation of the "cas-server-support-oauth":https://wiki.jasig.org/display/CASUM/OAuths module, which adds OAuth support to the "CAS server":http://www.jasig.org/cas (version 3.5.0).
...
ScribeUP is tested by 152 unit tests and also 7 integration tests on OAuth providers by simulating complete authentication processes (using the "HtmlUnit":http://htmlunit.sourceforge.net/ library).
