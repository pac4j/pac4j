...
| `RedirectAction getLogoutAction(WebContext context, CommonProfile currentProfile, String targetUrl)` | It returns the redirect action to call the identity provider logout.<br />The logout redirect action computation is done by a [`LogoutActionBuilder`](https://github.com/pac4j/pac4j/blob/master/pac4j-core/src/main/java/org/pac4j/core/logout/LogoutActionBuilder.java) |
{:.striped}
